  ...
  const cloudUpload = (uiArea) => {
    ...
    function ShowCloudUploadPopUp() {
      Swal.fire({
        ...
        showCloseButton: true,
        footer: '<div>由于云增加限制，目前周杰伦等无版权歌曲已经无法关联封面歌词。</div><div>建议先设置好请求头，以避免上传失败。</div><div><a href="https://github.com/Cinvin/myuserscripts/issues"  target="_blank">问题反馈</a></div>',
  ...
  const cloudUpgrade = (uiArea) => {
    ...
    function ShowCloudUpgradePopUp() {
      Swal.fire({
        ...
        confirmButtonText: "下一步",
        showCloseButton: true,
        footer: "<div>寻找云音源比云盘音质好的歌曲,然后进行删除并重新上传</div><div>⚠️可能会出现删除了歌曲但上传失败的情况</div><div>因此请自行做好备份</div><div>建议先设置好请求头，以避免上传失败但是文件被删除的情况。</div>",
        inputValidator: (value) => {
          if (!value) {
            return "请选择目标音质";
          }
